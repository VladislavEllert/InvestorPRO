...
Личный трекер инвестиций. Два артефакта в одном репозитории:

1. **`InvestorPro/`** — основное iOS-приложение (SwiftUI, Xcode). Цель проекта.
2. **`legacy_bot.py`** — старый Telegram-бот на Python (объединён в один файл). Источник истины по бизнес-логике
   (как тянуть данные T-Invest / Bybit, как считать PnL и доли). Переписываем на Swift, бот оставляем как референс.
